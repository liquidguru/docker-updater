...
- **Image cleanup** — opt-in toggle to remove the superseded image after a successful update (backup-aware: never removed while a kept backup still references it). Plus a "Show reclaimable images" list in Settings — see every dangling image with its size and age, select individual ones or all, and delete on demand; runs as a background job so a large batch can't be cut short by a proxy timeout, and any image that's still referenced is skipped with the actual blocking container named, not a generic error
