...
- **带健康校验的更新** — 在丢弃被替换的旧容器之前，会对新容器进行多次轮询检查；`restarting` 会被视为崩溃重启循环而非成功，如果镜像自带 HEALTHCHECK 且尚未通过，旧容器会作为备份保留而不会被删除
...
- **健康检查较为表层**：判断新容器是否正常，依据的是它的 Docker 状态（以及镜像自带的 HEALTHCHECK，如果有的话），而不是测试你的服务是否真正可用。一个进程正常运行、但持续返回错误的容器，与健康容器无法区分。如果镜像的健康检查需要较长时间才能通过，更新仍会生效，但旧容器会作为备份保留而不会被删除。
