...
> **注意：** 本项目由英文母语者维护，[英文 README](README.md) 为权威版本。中文翻译由社区贡献，可能落后于最新版本 —— 如有出入，请以英文版为准，特别是「注意事项」中与安全和数据相关的说明。欢迎提交 PR 更新翻译。
...
- **镜像仓库轮询** — 通过 Docker Registry v2 Manifest API（`HEAD` + `Docker-Content-Digest`）比较本地与远程镜像摘要，无需先拉取镜像；用于比较的是容器**实际正在运行**的镜像摘要，因此即使标签被外部的 `docker pull` 更新过、容器却没有重建，仍能正确检测到更新
...
- **单容器控制** — 可以单独更新、暂缓 7/14/30/90 天、无限期暂缓，或随时取消暂缓。暂缓期间该容器不会再出现在更新通知中，直到暂缓到期（通知中会说明有多少个被暂缓，因此不会有内容被悄悄遗漏）
- **批量更新** — 选择多个容器并一次发起更新
...
- **拉取过程中取消更新** — 镜像下载期间，容器卡片上会出现 **取消更新** 按钮。取消会中止下载，并且完全不会影响正在运行的容器；一旦拉取完成、开始重建容器，就没有安全的中断点了，因此该按钮仅在下载阶段生效
...
- **卡片显示镜像大小** — 每张容器卡片都会显示该镜像在磁盘上占用的大小；当有可用更新时，**↓** 标记会显示本次拉取实际需要下载的数据量（只统计本机尚未拥有的层），**▤ 新增/总数** 标记则显示层数，悬停可查看完整明细
...
- **安全重建** — 使用 Python Docker SDK 按 Watchtower 模式重建容器，沿用卷与命名挂载、端口、环境变量、网络、静态 IP 与别名、重启策略、Capabilities、设备、privileged/安全选项、DNS 与 extra hosts、PID/IPC 模式和 tmpfs（不会沿用的设置见[注意事项](#注意事项)）
- **备份与回滚** — 更新成功后可选择在指定时间内保留旧容器；如果新版本之后出现问题，可一键回滚，也可以提前删除备份以释放空间。远程主机同样适用：其备份会显示在“备份”页，并按时过期
- **镜像清理** — 可选地在更新成功后删除被替代的镜像；若备份仍引用该镜像则不会删除。设置页还提供“显示可回收镜像”，可查看所有悬空镜像的仓库名、大小和创建时间，选择部分或全部删除。配置了多台主机时，可通过主机选择器切换查看哪一台的镜像（镜像是按主机独立存储的）。删除在后台执行，避免大量镜像因代理超时而中断；仍被容器引用的镜像会显示具体阻止删除的容器
- **带健康校验的更新** — 在丢弃被替换的旧容器之前，会对新容器进行多次轮询检查；`restarting` 会被视为崩溃重启循环而非成功，如果镜像自带 HEALTHCHECK 且尚未通过，旧容器会作为备份暂时保留，直到健康检查通过后再自动删除——即使关闭了备份保留也是如此
- **安全备份** — docker-updater 创建的备份容器（`{name}_old`）会自动把重启策略设为 `no`，避免主机重启后正式容器和备份同时启动；回滚时会恢复原始策略，若回滚需要撤销则会再次重置
- **崩溃恢复** — 如果 docker-updater 在更新或回滚过程中重启，会在启动时协调遗留备份，并恢复任何被中断而处于停止状态的服务
- **隐藏辅助容器** — docker-updater 在操作过程中创建的容器（`{name}_old` 备份和 `{name}_rollingback`）不会出现在更新列表、镜像仓库检查和面板中
- **多架构镜像** — 发布 `linux/amd64` 和 `linux/arm64` 镜像，可运行在 x86 服务器和树莓派等 ARM 设备上
- **跳过本地构建镜像** — 由本地 Dockerfile 构建的镜像无法与镜像仓库比较，因此会被自动忽略。判定依据是「没有 `RepoDigests`、但仍保留着构建时使用的标签」，这样就不会把丢失了摘要的**已拉取**镜像误判为本地构建
...
每次检查更新时，**每个被跟踪的容器都会消耗一次 manifest 请求**。Docker Hub 会按 IP 对匿名请求做速率限制（目前为每小时 100 次），因此容器数量较多、检查频率较高，或多台机器共用同一个出口 IP 时，都可能耗尽配额。配额耗尽后，检查会开始失败，而不是报告更新。

使用账号认证可以提高该限制。请在 [Docker Hub → Account settings → Personal access tokens](https://app.docker.com/settings/personal-access-tokens) 创建一个**个人访问令牌**（只读权限即可，docker-updater 只会读取 manifest），然后设置：
...
如果显示 `[registry] Docker Hub: anonymous ...`，说明变量没有传入容器 —— 请注意 Compose 的 `.env` 文件只提供 `${VAR}` 替换，变量还必须列在 `environment:` 下（上文的 [Compose 示例](#docker-composeyml)已经包含）。

每次检查后都会记录剩余配额，配额较低时会给出警告：
...
- 凭据只会发送给 Docker Hub。GHCR、LSCR 等其他仓库不受影响，它们通常也有宽松得多的限制。
- 如果凭据被拒绝，docker-updater 会记录日志并回退到匿名检查，而不是直接失败。
- 以上只涉及**检查更新**。实际的 `docker pull` 由 Docker 守护进程使用它自己的凭据执行，因此如果拉取镜像也遇到限流，请在宿主机上另外执行 `docker login`。
...
2. 对每个容器，从 `RepoDigests` 中提取对应的本地镜像摘要 —— 读取的是容器**实际正在运行**的那个镜像，而不是标签当前指向的镜像，因此外部拉取导致标签前移、容器却未重建时，比较结果依然正确
...
5. 如果正在运行的镜像完全没有 `RepoDigests`（标签被外部拉取前移后遗留下来的镜像，既没有标签也没有摘要），则改为比较**镜像 ID**，通过解析当前平台的 Manifest Config 摘要来获取
6. 只有当前平台的本地镜像确实与镜像仓库中的最新镜像不同时，容器才会被标记为存在更新
7. 点击**更新**后，程序会：
   - 拉取新镜像，并把进度实时输出到日志弹窗
   ...
   - 等待 2 秒，并检查新容器是否仍在运行
   - **成功时**：删除 `_old` 容器；如果启用了备份保留，则在配置的时间内继续保留，以便之后回滚。如果健康检查尚未确认，`_old` 容器会一直保留到新容器报告健康后再删除（每 15 分钟检查一次，远程主机同样适用）；若新容器报告 `unhealthy`，则保留备份，详见[备份与回滚](#备份与回滚)
...
- **重建时不会保留的设置**：重建只会复制一份明确列出的配置项，不在该列表中的设置会回落到 Docker 守护进程的默认值。其中较为重要的有：**资源限制**（内存、CPU、PIDs）、**GPU/设备请求**、**显式设置的日志驱动**、**ulimits**、**sysctls**、只读根文件系统（`read-only`）、`--init`、自定义 `shm-size`、附加用户组，以及非默认 runtime。如果某个容器依赖上述设置，建议更新后改用 Compose 重建，或在更新后进行检查。（若某项设置本身就等于守护进程默认值，则不受影响 —— 例如在群晖上，使用默认 `db` 日志驱动的容器仍会保持该驱动。）
- **本地构建镜像**：由本地 Dockerfile 构建的镜像无法与镜像仓库比较，因此会被跳过。判定依据是「没有 `RepoDigests`、但仍保留着构建时使用的标签」—— 曾经**拉取过**、后来丢失了摘要的镜像（见下条）不会被跳过。
- **被外部拉取遗留的镜像**：如果其它工具或手动执行的 `docker pull` 拉取了容器正在使用的标签，标签会指向新镜像，而容器仍在运行旧镜像。根据镜像存储方式的不同，这个遗留镜像可能既没有标签也没有摘要。此时 docker-updater 会改为比较镜像 ID，因此容器仍会被正常标记，而不会被悄悄忽略。
- **私有镜像仓库**：支持匿名和 Bearer Token 镜像仓库，以及已认证的 Docker Hub（参见 [Docker Hub 速率限制](#docker-hub-速率限制)）。暂不支持为其他镜像仓库配置凭据 —— 如有需要，请提交 issue 并说明是哪个仓库。
- **新版本破坏性变更**：docker-updater 会保留容器原有的环境变量，但无法判断新镜像版本是否增加了新的必需环境变量。如果容器因此*崩溃*，会被检测到并自动回滚；但如果镜像能够正常启动、只是在应用层出现问题，则仍会通过健康检查，因此请查看镜像 Release Notes，确认是否新增了必需环境变量。
- **健康检查较为表层**：判断新容器是否正常，依据的是它的 Docker 状态（以及镜像自带的 HEALTHCHECK，如果有的话），而不是测试你的服务是否真正可用。一个进程正常运行、但持续返回错误的容器，与健康容器无法区分。如果镜像的健康检查需要较长时间才能通过（大多需要 10–60 秒，而更新过程只观察几秒钟），更新仍会生效，旧容器会暂时保留，直到健康检查通过后再删除；如果报告 `unhealthy`，则保留备份，方便你回滚。
- **Host 网络模式**：使用 `--network host` 的容器可以正确重建，此类容器会跳过网络重新连接步骤。
- **备份保留与磁盘空间**：启用备份保留后，每个备份都会保留一个已停止容器及旧镜像层，直到备份过期或在“备份”标签页中手动删除。磁盘空间有限时，请缩短保留时间，或在确认更新稳定后及时删除备份。另需注意：当新容器的健康状态无法确认时，**即使关闭了备份保留**，旧容器仍会被保留 —— 这是有意为之，以便始终有回退路径 —— 因此偶尔会出现你并未主动要求的 `_old` 容器。它会显示在“备份”标签页中，并按正常的过期时间清理。
